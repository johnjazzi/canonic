...
const mockApi = {
  ...
  workspace: { init: vi.fn().mockResolvedValue({ path: '/ws' }), getDefault: vi.fn(), openDialog: vi.fn(), openDirectoryDialog: vi.fn() },
  files: {
    list: vi.fn(async () => Object.entries(mockFiles).map(([p]) => ({ path: p, name: p.replace('.md', ''), type: 'file' }))),
    read: vi.fn(async (_, p) => mockFiles[p] ?? null),
    write: vi.fn(async (_, p, content) => { mockFiles[p] = content }),
    delete: vi.fn(async (_, p) => { delete mockFiles[p] }),
...
describe('document CRUD', () => {
  ...
  it('openFile() indexes content for search', async () => {
    ...
    expect(mockApi.search.index).toHaveBeenCalledWith('/ws', 'spec.md', '# Spec\n\nDetails here.')
  })

  it('openFile() loads associated comments', async () => {
    mockFiles['doc.md'] = '# Doc'
    mockApi.comments.get.mockResolvedValueOnce([{ id: 'c1', text: 'hello', resolved: false, anchor: {} }])
  ...
  it('commitFile() commits the current file and reloads log', async () => {
    ...
    mockApi.git.commit.mockResolvedValueOnce({ success: true, oid: 'deadbeef' })
    mockApi.git.log.mockResolvedValueOnce([{ oid: 'deadbeef', message: 'Add plan\n' }])
  ...
  it('renameFile() moves content to new path and updates currentFile', async () => {
    ...
    expect(mockApi.files.write).toHaveBeenCalledWith('/ws', 'new-name.md', '# Old name')
    expect(mockApi.files.delete).toHaveBeenCalledWith('/ws', 'old-name.md')
  })
  ...
  it('createBranch() creates branch and refreshes', async () => {
    ...
    mockApi.git.branches.mockResolvedValueOnce({ branches: ['main', 'feature'], current: 'main' })
