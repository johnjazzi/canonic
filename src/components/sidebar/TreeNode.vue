<template>
  <div>
    ...
    >
      ...
      <span v-if="item.type === 'file' && isDirty && !renaming" class="dirty-dot" />

      <!-- Hover actions -->
      <div v-if="hovered && !renaming" class="node-actions" @click.stop>
        <button v-if="item.type === 'directory'" class="node-btn" title="New file here" @click.stop="newFileHere">
          <FilePlus :size="11" />
        </button>
        <button v-if="item.type === 'directory'" class="node-btn" title="New folder here" @click.stop="newFolderHere">
          <FolderPlus :size="11" />
        </button>
        <button class="node-btn" title="Rename" @click.stop="startRename">
          <Pencil :size="11" />
        </button>
        <button v-if="item.type === 'file'" class="node-btn" title="Move to folder" @click.stop="showMove = !showMove">
          <ArrowRightFromLine :size="11" />
        </button>
        <button class="node-btn node-btn-danger" :title="item.type === 'directory' ? 'Delete folder' : 'Delete'" @click.stop="confirmDelete">
          <Trash2 :size="11" />
        </button>
      </div>
    </div>
    ...
    <div v-if="showMove" class="move-dropdown" @click.stop>
      ...
      <button class="move-opt" @click="doMove('')">(workspace root)</button>
      <button
    ...
    <div v-if="creatingFolder" class="inline-create" :style="{ paddingLeft: `${12 + (depth + 1) * 16 + 18}px` }">
      <input
        ref="folderInput"
        v-model="folderName"
        class="rename-input"
    ...
    <div v-if="creatingFile" class="inline-create" :style="{ paddingLeft: `${12 + (depth + 1) * 16 + 18}px` }">
      <input
        ref="fileInput"
        v-model="fileName"
        class="rename-input"
...
const isDirty = computed(() => store.currentFile === props.item.path && store.isDirty)
...
async function confirmRename() {
  ...
  if (props.item.type === 'file') {
    ...
    await window.canonic.files.move(store.workspacePath, props.item.path, newPath)
    await store.refreshFiles()
  }
}
...
async function confirmNewFile() {
  ...
  await window.canonic.files.write(store.workspacePath, filePath, `# ${name}\n\n`)
  await store.refreshFiles()
...
.dir-icon { flex-shrink: 0; color: var(--text-muted); }
...
.node-btn:hover { background: var(--bg-active); color: var(--text-primary); }
.node-btn-danger:hover { background: rgba(239,68,68,0.15); color: #f87171; }
...
.move-opt:hover { background: var(--bg-hover); color: var(--text-primary); }
.move-opt-current { opacity: 0.4; cursor: default; }
